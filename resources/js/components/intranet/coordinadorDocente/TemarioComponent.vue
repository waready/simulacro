            ...
            <div class="col-sm-12">
                <div class="card">
                    <header class="card-header">
                        Temarios
                        <button v-if="permissions.includes('agregar temarios')" class="btn btn-primary float-right" @click="nuevo"><i class="fa fa-plus"></i> Nuevo</button>
                    </header>
                    ...
                    <div class="card-body">
                        ...
                        <v-server-table ref="table" :columns="columns" :options="options" url="/intranet/coordinador-docente/temario/lista/data">
                            ...
                            <div slot="actions" slot-scope="props">
                                ...
                                <button v-if="permissions.includes('editar temarios')" class="btn btn-sm btn-info" @click="editar(props.row.id)">
                                    Editar
                                </button>
                                <button v-if="permissions.includes('pdf temarios')" class="btn btn-sm btn-warning" @click="ver(props.row.id)">
                                    Ver
                                </button>
...
export default {
    ...
    data() {
        return {
            ...
            columns: ["id", "periodo", "area", "curso", "actions"],
            options: {
                headings: {
                    id: "id",
                    periodo: "Periodo",
    ...
    methods: {
        nuevo: function() {
            this.edit = 0;
            this.errors = {};
            this.titulo = "Agregar Nuevo Temario";
            this.fields.area = "";
            this.fields.file = "";
        ...
        editar: function(id) {
            ...
            this.id = id;
            this.errors = {};
            this.titulo = "Editar Temario";
            axios.get("temario/" + id + "/edit").then(response => {
                this.cursos = response.data.cursos;
                this.fields.area = response.data.area.id;
                this.fields.curricula_detalle = response.data.temario.curricula_detalles_id;
        ...
        ver: function(id) {
            axios.get("temario/" + id).then(response => {
        ...
        submit: function() {
            ...
            if (this.edit == 0) {
                ...
                formData.append("file", typeof this.fields.file !== "undefined" ? this.fields.file : "");
                axios
                    .post("update-temario/" + this.id, formData)
                    .then(response => {
                        $(".loader").hide();
                        if (response.data.status) {
                            this.$refs.table.refresh();
